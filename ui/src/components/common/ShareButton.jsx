...
export default function ShareButton({ url, title }) {
  ...
  const getFullUrl = () =>
    typeof window !== "undefined" ? `${window.location.origin}${getPath()}` : getPath();
  ...
  const handleShare = async () => {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      try {
        await navigator.share({ title: getTitle(), url: fullUrl });
      } catch (err) {
  ...
  return (
    <div className="share-wrap" ref={wrapRef}>
      <button className="btn-icon" title="Share" onClick={handleShare} type="button">
        📤
      </button>
      ...
      {open && (
        <div className="share-menu">
          ...
          <a
            href={`https://www.threads.net/intent/post?text=${encodeURIComponent(`${getTitle()} ${getFullUrl()}`)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="share-icon"
            title="Threads"
          ...
          <a
            href={`https://wa.me/?text=${encodeURIComponent(`${getTitle()} ${getFullUrl()}`)}`}
            target="_blank"
            rel="noopener noreferrer"
            className="share-icon"
            title="WhatsApp"
